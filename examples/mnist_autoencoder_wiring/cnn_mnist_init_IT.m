...
block = dagnn.Conv('size', szl{k}, 'hasBias', true, 'stride', stridel{k}, 'pad', padl{k});
net.addLayer(lDTYb, block, {DTY}, {lDTYb}, {e_stepsizes{k}, e_biases{k}});

% ReLU
ReLU = ['e_layer_' num2str(k) '_iter_0_ReLU'];
net.addLayer(ReLU, dagnn.ReLU(), {lDTYb}, {ReLU}, {});
...
for t = 1:iters
    ...
    block = dagnn.Conv('size', szl{k}, 'hasBias', true, 'stride', 1, 'pad', 0 );
    net.addLayer(lDiff, block, {Diff}, {lDiff}, {e_stepsizes{k}, e_biases{k}});
    
    % Sum
    Sum = ['e_layer_' num2str(k) '_iter_' num2str(t) '_Sum'];
    block = dagnn.Sum();
    net.addLayer(Sum, block, {ReLU, lDiff}, Sum);
