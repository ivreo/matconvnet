...
% --------------------------------------------------------------------
function imdb = getPascalImdb2()
% --------------------------------------------------------------------
% Prepare the imdb structure, returns image data normalized to [0,1] 

data = [];
H = 300;
% Training dataset
datadir = '../pascal_data/VOCdevkit/VOC2012/JPEGImages/';
list = dir([datadir '2012_*.jpg']);
for i=1:length(list)
    tmp = imfinfo([datadir list(i).name]);
    sz = [tmp.Width, tmp.Height];
    if min(sz)>H
        im = single(imread([datadir list(i).name]));
        data = cat(4, data, sum(im(1:H,1:H,:),3)/3);
        % TODO (colorspace transformation) replace with Y component 
    end
end
Ltrain = size(data,4);
% Test dataset
