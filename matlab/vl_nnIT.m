...
mu = repmat(permute(mu,[2,3,1]),size(DTY,1),size(DTY,2),size(DTY,3),size(DTY,4));
...
for iter = 1 : iters
    est = vl_nnconv(Y, D, [], Gamma(:,:,:,:,iter), 'Pad', pad, 'Stride', stride, 'Dilate', dilate, 'NoDerFilters', opts{:}); % first arg not needed
