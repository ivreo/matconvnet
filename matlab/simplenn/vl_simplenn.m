...
%           [res(i).dzdx, dzdw{1}, dzdw{2}] = ...
%               vl_nnIT_der( ...
%               res(i).x, res(i+1).x, res(i+1).dzdx(:,:,:,:,end), ...
%               l.weights{1}, l.weights{2}, ...
%               l.stepsize, ...  % fixed constant stepsize
%               l.iters, ...
%               l.pad, ...
%               l.stride, ...
%               l.dilate, ...
%               cudnn);
          [res(i).dzdx, dzdw{1}, dzdw{2}, dzdw{3}] = ...
              vl_nnIT_der( ...
              res(i).x, res(i+1).x, res(i+1).dzdx(:,:,:,:,end), ...
              l.weights{1}, l.weights{2}, l.weights{3}, ...   % learned varying stepsize
              l.iters, ...
              l.pad, ...
              l.stride, ...
              l.dilate, ...
      ...
      case 'picklast'
        res(i).dzdx = zeros([size(res(i+1).dzdx),l.iters+1],'like',res(i+1).dzdx);
        res(i).dzdx(:,:,:,:,end) = res(i+1).dzdx;
